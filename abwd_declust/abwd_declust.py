...
def abwd_crjb(catalogue_pd, rupture_area_poly, rjb_cutoff, window_method, fs_time_prop):
    ...
    def crjb(rup1km_multipoint, rupture_area_centroid):
        crjb_final = []
    ...
    if window_method == 'GardnerKnopoff':
        ...
        sw_space[sw_space>=rjb_cutoff] = rjb_cutoff
    ...
    for i in range(0, neq - 1):
        if vcl[i] == 0:
            ...
            crjb_dist = crjb(rup1km_multipoint_sort[i], aftershock_centroids)
...
def abwd_rclosestp2h(catalogue_pd, rupture_area_poly, rjb_cutoff, window_method, fs_time_prop):
    ...
    def rclosest_p2h(grid_centroid_multi, epicentre_vsel):
        rclosest_p2h_final = []
    ...
    for i in range(0, neq - 1):
        if vcl[i] == 0:
            ...
            rclosest_p2h_dist = rclosest_p2h(grid_centroid_multi_sort[i], epicentre_vsel)
...
def abwd_rclosestp2p(catalogue_pd, rupture_area_poly, rjb_cutoff, window_method, fs_time_prop):
    ...
    def rclosest_p2p(rup1km_multipoint, rup1km_multipoint_ref):
        rclosest_p2p_final = []
    ...
    for i in range(0, neq - 1):
        if vcl[i] == 0:
            ...
            rclosest_p2p_dist = rclosest_p2p(rup1km_multipoint_sort[i], rup1km_multipoint_ref)
